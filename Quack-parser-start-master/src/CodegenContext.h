...
public:
    ...
    std::string alloc_reg() {
        ...
        object_code << "(obj_Obj) " << reg_name << ";" << std::endl;
        return reg_name;
    }


    void change_reg_type(int reg_num, int type){
      reg_type[reg_num] = type;
    }

    int get_reg_type(int reg_num){
      return reg_type[reg_num];
    }

    int get_current_reg_number(){
      return next_reg_num - 1;
    }

    std::string get_num_reg(int num){
      int reg_num = num;
      std::string reg_name = "tmp__" + std::to_string(reg_num);
      return reg_name;
    }
    ...
    std::string get_local_var(std::string &ident) {
        if (local_vars.count(ident) == 0) {
            ...
            this->emit(std::string("(obj_Obj) ") + internal + "; // Source variable " + ident);
            return internal;
        }
